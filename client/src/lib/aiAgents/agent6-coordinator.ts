...
function mockGenerateFinalReport(
  ...
  const optionRankings = generateOptionRankings(input, timeline, multiDimensional, probabilities);
...
function generatePros(timeline: any, multiDimensional: MultiDimensionalAnalysis, index: number): string[] {
...
function generateCons(timeline: any, multiDimensional: MultiDimensionalAnalysis, index: number): string[] {
  ...
  return [
    weakDimensions[0] ? `Challenges in ${weakDimensions[0].dimension} (${weakDimensions[0].score}/100)` : 'Requires significant adaptation',
    `${Math.round(Math.random() * 20 + 20)}% risk of major complications`,
    multiDimensional.tradeoffs[0]?.description || 'Trade-offs between competing priorities',
    'Uncertainty in long-term outcomes',
  ];
}

function generateBestFor(score: number, multiDimensional: MultiDimensionalAnalysis): string {
  if (score > 75) {
...
function generateActionPlan(topOption: OptionRanking, risks: RiskAnalysis): ActionStep[] {
  return [
    {
      step: 1,
...
function calculateConfidence(probabilities: ProbabilityTree, risks: RiskAnalysis): number {
  // 基于概率分布的集中度和风险严重性计算置信度
  const highProbPaths = probabilities.paths.filter(p => p.probability > 0.3).length;
