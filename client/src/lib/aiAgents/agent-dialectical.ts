...
export async function runDialecticalAnalysis(
  ...
  try {
    ...
    addLog({
      ...
      timestamp: Date.now(),
      thoughtProcess:
        "Scanning for positive externalities and compounding growth factors.",
    });
    ...
    const optimistView = await callDeepSeek(
      optimistPrompt,
      "You are an optimistic growth strategist.",
      {
    ...
    addLog({
      ...
      role: "optimist",
      message:
        "Optimistic projection complete. Identified significant upside in long-term scaling.",
      timestamp: Date.now(),
    });
    ...
    addLog({
      ...
      timestamp: Date.now(),
      thoughtProcess:
        "Applying Murphy's Law. Identifying single points of failure and hidden costs.",
    });
    ...
    const cynicView = await callDeepSeek(
      cynicPrompt,
      "You are a cynical risk manager and auditor.",
      {
        tier: MODEL_ROUTES.dialectical,
        temperature: 0.55,
      }
    ...
    addLog({
      ...
      role: "cynic",
      message:
        "Risk audit complete. Found 3 critical vulnerabilities in the optimistic model.",
      timestamp: Date.now(),
    });
    ...
    addLog({
      ...
      role: "synthesizer",
      message:
        "Synthesizing conflicting viewpoints into a balanced strategic framework...",
      timestamp: Date.now(),
      thoughtProcess:
        "Using Bayesian updating to reconcile the optimist and cynic views. Calculating weighted probabilities.",
    });
    ...
    const synthesisResponse = await callDeepSeek(
      synthesisPrompt,
      "You are a master synthesizer and strategic decision maker.",
      {
        tier: MODEL_ROUTES.dialectical,
        temperature: 0.4,
      }
    ...
    addLog({
      ...
      role: "synthesizer",
      message:
        "Strategic synthesis complete. Final recommendation formulated based on risk-adjusted returns.",
      timestamp: Date.now(),
    });
    ...
    console.info("Dialectical analysis using demo mode:", error);
    return mockDialecticalAnalysis(input, addLog);
  }
}

async function mockDialecticalAnalysis(
  input: DecisionInput,
  ...
  addLog({
    ...
    timestamp: Date.now(),
    thoughtProcess:
      "Analyzing market trends and potential for exponential growth.",
  });
  await delay(2000);
  ...
  addLog({
    ...
    timestamp: Date.now(),
    thoughtProcess:
      "Running 10,000 iterations to find the most robust path forward.",
  });
  await delay(3000);
